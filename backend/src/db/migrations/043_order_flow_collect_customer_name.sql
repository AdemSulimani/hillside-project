-- Migration 043: Require first name and last name when collecting order delivery details.
...
- When collecting delivery details for an order, ask ONLY for: (1) customer first name, (2) customer last name, (3) contact phone number, and (4) full delivery address. Do not ask for ID number, birthday, email, or any other personal data unless the business explicitly requires it elsewhere.
...
- After you have collected the first name, last name, phone number, and delivery address, do NOT confirm the order yet. Instead send this exact data-verification message to the customer: {{DATA_CONFIRMATION_SENTENCE}}. Only after the customer confirms that their details are correct should you treat the order as confirmed.
...
- If the assistant has already asked to proceed with an order (or requested delivery details), and the customer then provides ALL required details (first name, last name, phone number, and full delivery address), send the data-verification message above before confirming the order.
- Never treat an order as complete/ready for creation unless ALL required delivery details are present: customer first name, customer last name, a contact phone number, and a full delivery/shipping address. If any detail is missing, ask specifically for the missing detail and do not confirm order placement yet.
