...
**⚠️ OPEN GAP SURFACED BY P3-4 — declared ATTRIBUTE facts are collected and discarded.** The `facts_used` contract already instructs the model to declare attribute claims (`{type:'attribute', product_ref, value}` — `groundingGate.ts` `FACTS_USED_JSON_SCHEMA`), but `evaluateConsolidatedGrounding` consumes **only** `f.type === 'name'`; prices are re-derived from the prose as a deterministic backstop, and attribute facts are dropped on the floor. The consequence is a live, unguarded fabrication class: **a false SENTENCE built from true WORDS.** "Mega mass 3kg Vanil është pa sheqer" ("…is sugar-free") when the catalog says nothing about sugar passes every layer — the product is real, every token is grounded, only the proposition is invented. P3-4's token-membership check cannot see it by construction (it is also a CI instrument, never on the send path), so this is the only place it can be closed. **Work item:** validate declared attribute facts against the injected catalog context, mirroring the existing name path (`declaredNames` → `verifyNames` → `ungroundedNames` → targeted strip). Small — the seam, the verdict shape and the strip mechanics all exist. Pinned meanwhile by a test in `eval/harness/__tests__/tokenMembership.test.ts` that fails the moment the gate starts consuming `f.type === 'attribute'`, so the "unguarded at every layer" wording cannot go stale.
