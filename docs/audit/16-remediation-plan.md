...
**Validation.** **Determinism (primary gate):** the RC-01 golden set N≥20× → identical non-escalating decision; EV-011/013/015 → zero flags; IN1/IN3 stop escalating 8/8. `facts_used` property test: every cited price/name/claim present in the injected catalog facts; any citation absent from the full active catalog caught pre-send. Fan-out: completions/message drops from 18–25 to ≤2. Issue-2 depth: a 50+-turn conversation never re-asks a slot / never denies a recommended in-stock product. Billing reproducibility: identical order-ready transcripts → identical order + commissionability across N runs. The 548-suite + the standing eval harness (P3-4) green on every cutover PR.
