    ...
    <div className="space-y-6 pb-10">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Flag incorrect AI replies in the inbox and track corrections for future fine-tuning.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Feedback items recorded for your workspace.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {configQuery.isLoading ? (
              ...
              <p className="text-sm text-destructive">
                {extractMessage(configQuery.error, 'Could not load progress.')}
              </p>
            ) : (
              <>
      ...
      {listQuery.isLoading ? (
        ...
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6 text-sm text-destructive">
          {extractMessage(listQuery.error, 'Could not load feedback logs.')}
        </div>
      ) : logs.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-16 text-center">
          <MessageSquareHeart className="size-10 text-muted-foreground opacity-50" />
          <p className="text-sm text-muted-foreground">
            No feedback entries yet. Use the thumbs-down control on an AI message in the inbox.
          </p>
        </div>
      ) : (
        <>
                ...
                {logs.map((row) => (
                  <tr key={row.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-3 py-2.5 align-top text-muted-foreground">
                      <CellText text={row.reason} empty="—" />
                    </td>
                    <td className="px-3 py-2.5 align-top">
